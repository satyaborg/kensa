...
class TestJudgeScenarioTrajectoryMetrics:
    def test_trajectory_metrics_promoted_on_pass(self, sample_spans: list[Span]) -> None:
        from kensa.models import Check, CheckType

        scenario = Scenario(
            id="traj_pass",
            name="Trajectory pass",
            run_command=["python", "agent.py"],
            checks=[
                Check(
                    type=CheckType.TRAJECTORY,
                    params={"steps": [{"tool": "get_weather", "args": {"city": "San Francisco"}}]},
                )
            ],
        )
        result = judge_scenario(scenario, sample_spans, "traces/t.jsonl")
        assert result.status == ResultStatus.PASS
        assert result.metrics["trajectory_accuracy"] == 1.0
        assert result.metrics["step_efficiency"] == 1.0

    def test_trajectory_metrics_present_when_check_fails(self, sample_spans: list[Span]) -> None:
        from kensa.models import Check, CheckType

        scenario = Scenario(
            id="traj_fail",
            name="Trajectory fail",
            run_command=["python", "agent.py"],
            checks=[
                Check(
                    type=CheckType.TRAJECTORY,
                    params={"steps": [{"tool": "search_docs"}]},
                )
            ],
            criteria="This should never run because the deterministic check fails.",
        )
        mock = MockJudge()
        result = judge_scenario(scenario, sample_spans, "traces/t.jsonl", mock)
        assert result.status == ResultStatus.FAIL
        assert mock.call_count == 0
