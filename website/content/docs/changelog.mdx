...
- Slug-normalization tests are now derived from the live OpenRouter catalog, so regressions surface against real model IDs.
...
- PyPI publishing uses `skip-existing` so partial uploads are recoverable on retry.

## [0.5.0](https://github.com/satyaborg/kensa/releases/tag/v0.5.0) — 2026-04-15

The full eval workflow is now available over MCP, so any MCP-aware client can drive kensa as tools.
...
- Docs updated around the MCP launch.
