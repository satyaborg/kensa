...
| `input` | No | The prompt or input to the agent. Omit when the agent reads its own input or when the scenario uses `dataset`. |
| `run_command` | Yes | Argv list passed to `subprocess.run` (no shell). When `input` is set, it is appended as the final argv element. |
