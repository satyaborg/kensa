...
`uvx kensa init` is now a one-shot bootstrap: scaffold scenarios, add kensa as a dev dep, and drop the skills into whichever coding agent you use.

- New `kensa skills install` command copies the bundled skills into `.claude/skills/` (Claude Code) and `.agents/skills/` (Codex, Cursor, OpenCode, Gemini CLI, and other adopters of the Agent Skills standard). Use `--global` to write under `~`, `-a <agent>` to scope to a specific target, and `--force` to overwrite existing files.
- `kensa init` gained `--cli` / `--no-cli` for the dev-dep step and `-a/--agent` for picking the coding agent (`claude`, `codex`, `cursor`, `opencode`, `gemini`, `other`, `all`, `none`). In an interactive terminal, the agent step prompts (defaulting from existing project markers); in CI the agent default is `none` so nothing is installed unless `-a` is passed.
...
Release tooling and a docs link fix.

- Fixed the MCP server documentation 404'ing: Mintlify reserves `/docs/mcp` for its own manifest, so the page is now served at `/docs/mcp-server` and all internal links updated.
