...
`uvx kensa init` adds `kensa` to your dev deps, scaffolds `.kensa/`, and prompts you to choose which coding agent to install skills for. Use `kensa skills install` later to refresh after a kensa upgrade.
...
```
Setup (audit-evals) → Capture (kensa capture) → Design (generate-scenarios)
  → Calibrate (generate-judges) → Validate (validate-judge) → Execute (kensa eval)
  → Diagnose (diagnose-errors) → Iterate
```

`generate-scenarios` mines whatever traces it can find. When `kensa capture -- <cmd>` produced the most recent run, the synthesized scenarios reuse the captured `run_command` and (with `-i`) the captured input verbatim, so they replay against the same entrypoint without manual editing.
