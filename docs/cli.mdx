...
`--example` writes `.kensa/agents/example.py` and `.kensa/scenarios/example.yaml`; run `kensa eval` afterwards to verify your setup. Without `--example`, `kensa init` leaves the directory empty so you (or your coding agent) can author scenarios from scratch.
